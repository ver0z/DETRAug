...
class AugMix(object):
    ...
    def __call__(self, image, target):
        if random.random() < self.prob:
            image, target = augmix.augment_and_mix.augment_and_mix(np.asarray(image)/255, denormalize_image=False, positional_aug=self.include_pos_aug_mix, target = target, modify_target_boxes = self.modify_target_boxes)
